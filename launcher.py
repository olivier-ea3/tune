...
def set_transparent_huge_pages(tbh_value, elevation_pwd=None):
    ...
    if os.path.exists(THP_LOCATION):
        ...
        code = subprocess.call(
            f'echo {elevation_pwd} | sudo -S sh -c "sync;echo 3 > /proc/sys/vm/drop_caches"',
            shell=True,
        )
        if code != 0:
        ...
        code = subprocess.call(
            f'echo {elevation_pwd} | sudo -S sh -c "echo {tbh_value} > {THP_LOCATION}"',
            shell=True,
        )
        if code != 0:
...
def set_multi_thread_and_allocator(args):
    ...
    if args.enable_thp:
        SUDOER_PASSWORD = getpass(
            "Setting Transparent Huge Page requires elevated privileges.\nPassword:"
        )
...
def launch(args):
    ...
    args.additional_benchmark_args.append(
        f"experiment_id={os.environ['EXPERIMENT_ID']}"
    )
